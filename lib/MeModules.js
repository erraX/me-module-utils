...
class MeModules {
  ...
  diggSelfExports() {
    const diggFn = (mod, name) => {
      ...
      const code = (0, _utils.safeReadFileSync)(absolutePath); // 模块虽然定义了，但是文件不存在
      ...
      mod.selfExports = (0, _utils.getMatchedExportsName)(code);
    };
  ...
  diggCss() {
    this.each((mod, name) => {
      ...
      mod.css = css.map(src => (0, _path.join)(basePath, src));
    });
  }
  ...
  flattenModuleDeps(name, mod, deps = {}) {
    ...
    if (mod.src !== 'lib/me.js' && req.indexOf('mejs') === -1 && mod.isMeModule) {
      req.push('mejs');
    }
  ...
  each(iter) {
    ...
    return Object.keys(modules).some(name => iter(modules[name], name));
  }
